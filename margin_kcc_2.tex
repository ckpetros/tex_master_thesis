...
Fitting the data to a \ac{GAMLSS} model like this can be considered favourable. The emerged residuals are closer to a standard normal distribution than the maximum likelihood estimation without any covariate effects. Diagnostic plots of quantile residuals in \autoref{fig:gamlss_residuals_kcc_2} confirm this. R output \ref{output:gamlss_residuals_kcc_2} below returns a detailed summary of the estimated location, scale and shape parameters of the residuals' distribution. A Shapiro-Wilk test for the newly emerged residuals returns a p-value of 0.92, which is in plain favour of a normal distribution.
\\
